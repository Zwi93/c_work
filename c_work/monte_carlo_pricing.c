...
void statistical_analysis (double *array, int size_of_array, int no_of_trials, double rate, double vol, double time_step, double spot_price, double strike);
...
int main ()
{
    ...
    //statistical_analysis(random_walk_array, STEPS, TRIALS, RATE, VOLATILITY, TIME_STEP, SPOT_PRICE, STRIKE);
    clock_t end = clock();
...
double monte_carlo_estimate (int no_of_simulation, double *array, int size_of_array, int no_of_trials, double rate, double vol, double time_step, double spot_price, double strike)
{
    ...
    for (index = 0; index < no_of_simulation; index++)
    {
        ...
        payoff = forward_payoff_function(array[size_of_array - 1], spot_price, rate, expiry);
        estimate += payoff;
    }
...
void statistical_analysis (double *array, int size_of_array, int no_of_trials, double rate, double vol, double time_step, double spot_price, double strike)
{
    //Function to perform statistical analysis on the Monte Carlo scheme; find the average, variance, etc and compare with exact result.
    FILE *my_stream;
    my_stream = fopen("stats.txt", "w");
    fprintf(my_stream, "Simu Average Std-Div\n");

    double binary_price;
    int expiry = time_step*size_of_array;
    binary_price = binary_option_price (spot_price, strike, expiry, rate, vol);

    int outer_index;
    for (outer_index = 0; outer_index < 20; outer_index++)
    {
        int simulations;
        simulations = 1000 + 10*outer_index;

        int index;
        double mean_estimate = 0;
        double std_estimate = 0;

        for (index = 0; index < 100; index++)
        {
            double estimate;
            estimate = monte_carlo_estimate (simulations, array, size_of_array, no_of_trials, rate, vol, time_step, spot_price, strike);
            mean_estimate += estimate;
            std_estimate += pow((estimate - binary_price), 2); 
        }

        mean_estimate = mean_estimate/100;
        std_estimate = sqrt(std_estimate/100);

        fprintf(my_stream, "%d %f %f\n", simulations, mean_estimate, std_estimate);
    }
